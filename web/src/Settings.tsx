  ...
  renderWebSection() {
    return (
      <Row className="g-3 align-items-center">
        ...
        <label htmlFor="webPort" className="col-sm-3 col-form-label">
          Port Number:
        </label>
        <Col sm={9}>
          <input
            ...
            value={this.state.settings.web.port}
            onChange={this.handleWebPortChange.bind(this)}
          />
        </Col>
      </Row>
    );
  ...
  renderHotkeySection(keyboardCodes: KeyboardCodes) {
    ...
    return (
      <div className="row g-3 mt-5 align-items-center">
        ...
        <label htmlFor="hotkeyNextHostKey1" className="col-sm-4 col-form-label">
          Next Host Hotkey:
        </label>
        <div className="col-sm-8">
          <select
            className="form-select"
            id="hotkeyNextHostKey1"
            value={this.state.settings.hotkeys.nextHost[1]}
            onChange={this.handleHotkeyChange.bind(this)}
          >
      ...
      .then((response) => {
        if (response.ok) {
          this.context.addNotification(
            NotifyType.success,
            'Restart ' + serviceName + ' service requested successfully'
          );
        } else {
          this.context.addNotification(
            NotifyType.error,
            'Something went wrong during restart ' + serviceName + ' service send...'
          );
        }
      })
      .catch((error) => {
